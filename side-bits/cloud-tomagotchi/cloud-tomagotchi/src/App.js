...
function update(DBdata) {
  ...
  axios.post('http://localhost:4000/'+DBdata._id, DBupdate)
}
...
function del () {
  axios.get('http://localhost:4000/')
   .then(response => {
    ...
    DBdata.forEach(DBpet => {
        if(DBpet.Petname === document.getElementById('name').value){
          axios.delete('http://localhost:4000/'+DBpet._id)
        }
    })
   })
...
function load (DBdata) { 
 ...
 function save () { 
  const DBdata = {
   ...
   axios.post('http://localhost:4000/',DBdata)
        ...
        <div className="ui">
            ...
            <div className="bottom-ui ui-comp">
                ...
                <div className="settings">
                    ...
                    <button className="setting btn" id="muteBtn" onClick={cloud}>
                    </button>
                </div>
